  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        kind: 'p',
        text:
          'The auto-invoice setting (Billing settings) decides what happens when a visit finishes. "Invoice each visit" (the default) creates and sends an invoice for exactly that visit — and its payment section rides the report page the client already got, so there is no second email. "Add to open draft" collects finished visits onto one draft you review and send. "Manual" leaves it all to you. While an invoice is still a draft, its "Issued on" date is editable — backdate it if the invoice should carry an earlier date. That backdating is also the key to switching from another platform: for a client who still owes you from before the switch, create an invoice dated your migration day with one manual line ("Balance carried over from <old platform>") and send it — it shows in their balance, on their statement as day-one history, and they can pay it like any other invoice. For a client whose money YOU hold, record a deposit with a memo saying where it came from — it shows as "Holding" and applies to their next invoices automatically. A walker owed wages from before the switch gets a payout statement with one manual adjustment line. (Tax note: carried-over balances are old receivables, not new revenue — tell your accountant.)',
      },
      {
        kind: 'steps',
        items: [
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        kind: 'p',
        text:
          'Wondering if you have the latest features? Settings → App version shows exactly which update you\u2019re running, and "Check for updates" downloads and applies the newest one on the spot. (If your phone flashes a "crashed" message right after the restart, that\u2019s just how iOS logs the relaunch — nothing is wrong.) And if the app ever truly crashes, it reports the details to support automatically — you don\u2019t need to do anything, though screenshots of what you were doing always help.',
      },
      {
        kind: 'p',
