...
function ActiveVisitBody() {
  ...
  const offerClientText = (detailNow: VisitDetail) => {
    // Land back in the CALLER'S shell: a dual-role owner finishing their own
    // walk returns to the owner Today, never the walker one (tab-shell
    // unification, 2026-09-06).
    const ownerHere =
      memberships.data?.find((m) => m.business_id === detailNow.visit.business_id)?.role ===
      'owner';
    const goToday = () =>
      router.replace((ownerHere ? '/(owner)/today' : '/(walker)/today') as Href);
